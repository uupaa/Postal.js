...
Postal["prototype"] = Object.create(Postal, {
    ...
    "remove":           { "value": Postal_remove        }, // Postal#remove(receiver:ReceiverObject):Envelope
    "first":            { "value": Postal_first         }, // Postal#first(message:String, param:Any = undefined, ...):Any|undefined
...
Envelope["prototype"] = Object.create(Envelope, {
    ...
    "remove":           { "value": Postal_remove        }, // Envelope#remove(receiver:ReceiverObject):Envelope
    "first":            { "value": Postal_first         }, // Envelope#first(message:String, param:Any = undefined, ...):Any|undefined
...
//{@dev
    if (VERIFY) {
        $valid($type(receiver["inbox"], "Function"), Postal_id, "receiver");
    }
//}@dev
...
//{@dev
    if (VERIFY) {
        if (receiver) {
            $valid($type(receiver["inbox"], "Function"), Postal_unregister, "receiver");
        }
    }
//}@dev
...
//{@dev
    if (VERIFY) {
        if (receiver) {
            $valid($type(receiver["inbox"], "Function"), Postal_to, "receiver");
        }
    }
//}@dev
...
function Postal_remove(receiver) { // @arg ReceiverObject - remove receiver.
                                   // @ret Envelope
                                   // @desc remove receiver.
//{@dev
    if (VERIFY) {
        $valid($type(receiver["inbox"], "Function"), Postal_remove, "receiver");
    }
//}@dev
...
function Postal_first(message       // @arg String - message
                      /*, ... */) { // @var_args Any - inbox params. inbox(message, param, ...)
                                    // @ret Any|undefined - a result value.
                                    // @desc send a message synchronously. first-come, first-served.
//{@dev
    if (VERIFY) {
        $valid($type(message, "String"), Postal_first, "message");
        $valid(message.length,           Postal_first, "message");
    }
//}@dev
    ...
    for (var i = 0, iz = to.length; i < iz; ++i) {
        ...
        if (receiverObject) { // alive?
            var r = receiverObject["inbox"].apply(receiverObject, args);
